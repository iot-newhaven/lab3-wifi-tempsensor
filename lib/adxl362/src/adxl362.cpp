...
//
//  begin()
//  Initial SPI_PORT setup, soft reset of device
//
void ADXL362::begin(int16_t chipSelectPin) {
	slaveSelectPin = chipSelectPin;
	pinMode(slaveSelectPin, OUTPUT);

	SPI_PORT.begin(SPI_MODE_MASTER);
    //SPI_PORT.setBitOrder(MSBFIRST);
    SPI_PORT.setClockSpeed(1,MHZ);

	SPI_PORT.setDataMode(SPI_MODE0);	//CPHA = CPOL = 0    MODE = 0
	delay(1000);
...
void ADXL362::readXYZTData(int16_t &XData, int16_t &YData, int16_t &ZData, int16_t &Temperature){
	  ...
	  SPI_PORT.transfer(0x0B);  // read instruction
	  SPI_PORT.transfer(0x0E);  // Start at XData Reg
	  XData = SPI_PORT.transfer(0x00);
	  XData = XData + (SPI_PORT.transfer(0x00) << 8);
	  YData = SPI_PORT.transfer(0x00);
	  YData = YData + (SPI_PORT.transfer(0x00) << 8);
	  ZData = SPI_PORT.transfer(0x00);
	  ZData = ZData + (SPI_PORT.transfer(0x00) << 8);
	  Temperature = SPI_PORT.transfer(0x00);
	  Temperature = Temperature + (SPI_PORT.transfer(0x00) << 8);
...
void ADXL362::checkAllControlRegs(){
	...
	digitalWrite(slaveSelectPin, LOW);
	SPI_PORT.transfer(0x0B);  // read instruction
	SPI_PORT.transfer(0x20);  // Start burst read at Reg 20
#ifdef ADXL362_DEBUG
	Serial.println("Start Burst Read of all Control Regs - Library version 6-5-2014:");
	Serial.print("Reg 20 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 21 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 22 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 23 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 24 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 25 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 26 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 27 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 28 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 29 = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 2A = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 2B = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 2C = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 2D = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
	Serial.print("Reg 2E = "); 	Serial.println(SPI_PORT.transfer(0x00), HEX);
#endif
	digitalWrite(slaveSelectPin, HIGH);
}

// Basic SPI_PORT routines to simplify code
...
byte ADXL362::SPIreadOneRegister(byte regAddress){
	...
	digitalWrite(slaveSelectPin, LOW);
	SPI_PORT.transfer(0x0B);  // read instruction
	SPI_PORT.transfer(regAddress);
	regValue = SPI_PORT.transfer(0x00);
	digitalWrite(slaveSelectPin, HIGH);
...
void ADXL362::SPIwriteOneRegister(byte regAddress, byte regValue){
	...
	digitalWrite(slaveSelectPin, LOW);
	SPI_PORT.transfer(0x0A);  // write instruction
	SPI_PORT.transfer(regAddress);
	SPI_PORT.transfer(regValue);
	digitalWrite(slaveSelectPin, HIGH);
}
...
int16_t ADXL362::SPIreadTwoRegisters(byte regAddress){
	...
	SPI_PORT.transfer(0x0B);  // read instruction
	SPI_PORT.transfer(regAddress);  
	twoRegValue = SPI_PORT.transfer(0x00);
	twoRegValue = twoRegValue + (SPI_PORT.transfer(0x00) << 8);
	digitalWrite(slaveSelectPin, HIGH);
...
void ADXL362::SPIwriteTwoRegisters(byte regAddress, int16_t twoRegValue){
	...
	SPI_PORT.transfer(0x0A);  // write instruction
	SPI_PORT.transfer(regAddress);  
	SPI_PORT.transfer(twoRegValueL);
	SPI_PORT.transfer(twoRegValueH);
	digitalWrite(slaveSelectPin, HIGH);
}
